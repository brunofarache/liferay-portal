		...
		<c:choose>
			...
			<c:otherwise>
				<liferay-frontend:empty-result-message
					actionDropdownItems="<%= ddmFormAdminDisplaySearchContainer.isSearch() ? null : ddmFormAdminDisplayContext.getAddButtonDropdownItems() %>"
					animationType="<%= ddmFormAdminDisplaySearchContainer.isSearch() ? EmptyResultMessageKeys.AnimationType.SUCCESS : EmptyResultMessageKeys.AnimationType.EMPTY %>"
					description='<%= ddmFormAdminDisplaySearchContainer.isSearch() ? "" : LanguageUtil.get(request, "create-and-save-an-element-set") %>'
					title='<%= ddmFormAdminDisplaySearchContainer.isSearch() ? LanguageUtil.get(request, "no-element-sets-were-found") : LanguageUtil.get(request, "there-are-no-element-sets") %>'
				/>
			</c:otherwise>
		</c:choose>
	</aui:form>
