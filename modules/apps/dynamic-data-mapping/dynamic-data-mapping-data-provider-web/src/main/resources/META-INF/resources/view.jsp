...
SearchContainer<?> ddmDataProviderDisplaySearchContainer = ddmDataProviderDisplayContext.getSearch();
%>
...
>
	<aui:form action="<%= portletURL.toString() %>" method="post" name="searchContainerForm">
		...
		<c:choose>
			<c:when test="<%= ddmDataProviderDisplaySearchContainer.hasResults() %>">
				<liferay-ui:search-container
					id="<%= ddmDataProviderDisplayContext.getSearchContainerId() %>"
					rowChecker="<%= new EmptyOnClickRowChecker(renderResponse) %>"
					searchContainer="<%= ddmDataProviderDisplaySearchContainer %>"
				>
			...
			<c:otherwise>
				...
				<%
				boolean isSearch = ddmDataProviderDisplaySearchContainer.isSearch();
				%>

				<liferay-frontend:empty-result-message
					actionDropdownItems="<%= isSearch ? null : ddmDataProviderDisplayContext.getAddDataProviderDropdownItems() %>"
					animationType="<%= isSearch ? EmptyResultMessageKeys.AnimationType.SUCCESS : EmptyResultMessageKeys.AnimationType.EMPTY %>"
					description='<%= isSearch ? "" : LanguageUtil.get(request, "create-and-save-a-data-provider") %>'
					title='<%= isSearch ? LanguageUtil.get(request, "no-data-providers-were-found") : LanguageUtil.get(request, "there-are-no-data-providers") %>'
				/>
			</c:otherwise>
		</c:choose>
	</aui:form>
