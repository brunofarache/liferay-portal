...
public class DataEngineExpandoBridgeImpl implements ExpandoBridge {
		...
		try {
			if (Validator.isNull(dataDefinition.getId())) {
				dataDefinitionResource.postSiteDataDefinitionByContentType(
					_companyGroupId, "intrinsic-object", dataDefinition);
			}
		}
		catch (Exception exception) {
			throw new RuntimeException(exception);
	...
	public UnicodeProperties getAttributeProperties(String name) {
		...
		for (DataDefinitionField dataDefinitionField :
			...
			if (Objects.equals(name, dataDefinitionField.getName())) {
	...
	public Map<String, Serializable> getAttributes(
		...
		List<String> filteredAttributeNames = stream.filter(
			attributeName -> ListUtil.exists(
				new ArrayList<>(names), attributeName::equals)
		).collect(
			Collectors.toList()
		);

		for (String attributeName : filteredAttributeNames) {
			attributes.put(attributeName, getAttribute(attributeName));
		}
	...
	public void setAttributes(
		...
		try {
			for (Map.Entry<String, Serializable> entry :
				...
				addAttribute(
					entry.getKey(), ExpandoColumnConstants.STRING,
					entry.getValue());
			}
		}
	...
	protected DataEngineExpandoBridgeImpl(long companyId, String className) {
		this(companyId, className, 0);
	}

	protected DataEngineExpandoBridgeImpl(
		long companyId, String className, long classPK) {

		this(
			companyId, className, classPK,
			DDLRecordLocalServiceUtil.getService(),
		...
		try {
			...
			return dataDefinitionResource.
				getSiteDataDefinitionByContentTypeByDataDefinitionKey(
					_companyGroupId, "intrinsic-object", _className);
		}
		catch (Exception exception) {
			throw new RuntimeException(exception);
		}
