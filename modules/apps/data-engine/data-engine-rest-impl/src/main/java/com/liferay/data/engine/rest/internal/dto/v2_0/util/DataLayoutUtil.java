...
public class DataLayoutUtil {
	...
	private static DataRule[] _toDataRules(
		...
		for (SPIDDMFormRule ddmFormRule :
			...
			dataRule.setActions(
				Stream.of(
					ddmFormRule.getSPIDDMFormRuleActions()
				).flatMap(
					Collection::stream
				).map(
					ddmFormRuleAction -> gson.fromJson(
						JSONFactoryUtil.looseSerializeDeep(ddmFormRuleAction),
						Map.class)
				).toArray(
					Map[]::new
				));
			...
			dataRule.setConditions(
				Stream.of(
					ddmFormRule.getSPIDDMFormRuleConditions()
				).flatMap(
					Collection::stream
				).map(
					ddmFormRuleCondition -> gson.fromJson(
			...
			dataRule.setLogicalOperator(ddmFormRule.getLogicalOperator());
